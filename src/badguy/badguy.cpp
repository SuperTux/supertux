...
BadGuy::update(float elapsed_time)
{
  if (!Sector::current()->inside(bbox)) {
    ...
    + "] "; log_warning << current_level << "Counted badguy " << badguy << "
    starting at " << start_position << " has left the sector" <<std::endl;;
    }*/
    return;
  }
  ...
  switch (state) {
    ...
    case STATE_MELTING: {
      ...
      if (sprite->animation_done() || on_ground()) {
        Sector::current()->add_object(std::make_shared<WaterDrop>(
            bbox.p1, get_water_sprite(), physic.get_velocity()));
        remove_me();
        break;
      }
    } break;
    ...
    case STATE_INSIDE_MELTING: {
      ...
      Sector::current()->add_object(std::make_shared<SpriteParticle>(
          get_water_sprite(), "particle_" + std::to_string(pa), ppos,
          ANCHOR_MIDDLE, Vector(0, 0),
          Vector(0, 100 * Sector::current()->get_gravity()),
          LAYER_OBJECTS - 1));
    } break;
    case STATE_FALLING:
      is_active_flag = false;
...
BadGuy::is_offscreen() const
{
  ...
  // was approx. <= ~668px This doesn't work for wide-screen monitors which give
  // us a virt. res. of approx. 1066px x 600px
