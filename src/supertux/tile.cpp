...
void
Tile::draw(DrawingContext& context, const Vector& pos, int z_pos,
           Color color) const
{
...
Tile::print_debug(int id) const
{
  ...
  for (const auto& im : editor_imagespecs)
    log_debug << "  Editor Imagespec: file " << im.file << "; rect " << im.rect
              << std::endl;
  for (const auto& im : imagespecs)
    log_debug << "  Imagespec:        file " << im.file << "; rect " << im.rect
              << std::endl;
}
