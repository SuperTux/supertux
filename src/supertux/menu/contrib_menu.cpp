...
ContribMenu::ContribMenu() : m_contrib_worlds()
{
  ...
  std::unique_ptr<char*, decltype(&PHYSFS_freeList)> addons(
      PHYSFS_enumerateFiles("custom"), PHYSFS_freeList);
  for (const char* const* addondir = addons.get(); *addondir != 0; ++addondir) {
    ...
    if (PhysFSFileSystem::is_directory(addonpath)) {
      std::string addonlevelpath =
          FileSystem::join(addonpath.c_str(), "levels");
      if (PhysFSFileSystem::is_directory(addonlevelpath)) {
        std::unique_ptr<char*, decltype(&PHYSFS_freeList)> addonfiles(
            PHYSFS_enumerateFiles(addonlevelpath.c_str()), PHYSFS_freeList);
        for (const char* const* filename = addonfiles.get(); *filename != 0;
             ++filename) {
          std::string filepath =
              FileSystem::join(addonlevelpath.c_str(), *filename);
          if (PhysFSFileSystem::is_directory(filepath)) {
            level_worlds.push_back(filepath);
          }
        }
  ...
  for (std::vector<std::string>::const_iterator it = level_worlds.begin();
    ...
    try {
      auto levelset = std::unique_ptr<Levelset>(
          new Levelset(*it, /* recursively = */ true));
      if (levelset->get_num_levels() == 0) continue;
