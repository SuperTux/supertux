...
class Tile {
 ...
 public:
  ...
  Tile(const std::vector<ImageSpec>& images,
       ...
       uint32_t data, float fps, const std::string& obj_name = "",
  ...
  void draw(DrawingContext& context, const Vector& pos, int z_pos,
            Color color = Color(1, 1, 1)) const;
  ...
  bool is_slope() const { return attributes & SLOPE; }
  ...
  bool is_solid() const { return attributes & SOLID; }
   ...
   */
  bool is_unisolid() const { return attributes & UNISOLID; }

  std::string get_object_name() const { return object_name; }

  std::string get_object_data() const { return object_data; }
