...
void PhysfsSubsystem::find_mount_datadir()
{
  ...
  if (!PHYSFS_mount(boost::filesystem::canonical(m_datadir).string().c_str(), nullptr, 1))
  {
...
{
#ifndef __EMSCRIPTEN__
  if (!PHYSFS_mount(FileSystem::join(boost::filesystem::canonical(m_datadir).string(), dir).c_str(), dir.c_str(), 0))
  {
...
if (FileSystem::is_directory(olduserdir)) {
  ...
  boost::filesystem::path userpath(m_userdir);
