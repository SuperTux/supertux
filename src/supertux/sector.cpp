...
{
  auto source_desc = "Sector " + name + " - " + sourcename;
  try {
    return scripting::run_script(in, source_desc, scripts, "sector");
  }
  catch(const std::exception& e)
  {
    log_warning << "Error running sector script " << source_desc << ": " << e.what() << std::endl;
    return NULL;
  }
}
