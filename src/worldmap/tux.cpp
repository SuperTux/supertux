...
{
  std::string action = get_action_prefix_for_bonus(
      worldmap->get_savegame().get_player_status()->bonus);
  if (!action.empty()) {
...
{
  if (bonus == GROWUP_BONUS) return "large";
  if (bonus == FIRE_BONUS) return "fire";
  if (bonus == ICE_BONUS) return "ice";
  if (bonus == AIR_BONUS) return "air";
  if (bonus == EARTH_BONUS) return "earth";
  if (bonus == NO_BONUS) return "small";
