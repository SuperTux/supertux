    ...
    if (m_parent.m_enter_level && !m_tux->is_moving())
    {
      ...
      if (level_->get_tile_pos() == m_tux->get_tile_pos()) {
        try {
          ...
          m_parent.save_state();
          GameManager::current()->start_worldmap_level(levelfile, m_parent.m_savegame, &level_->get_statistics());
          ScreenManager::current()->set_screen_fade(std::make_unique<ShrinkFade>(shrinkpos, 1.0f, LAYER_LIGHTMAP - 1));
