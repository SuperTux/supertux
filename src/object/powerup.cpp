...
PowerUp::PowerUp(const ReaderMapping& lisp)
    : MovingSprite(lisp, "images/powerups/egg/egg.sprite", LAYER_OBJECTS,
                   COLGROUP_MOVING),
      physic(),
      script(),
      no_physics(),
...
PowerUp::get_settings()
{
  ...
  result.options.push_back(ObjectOption(MN_TOGGLE, _("Disable gravity"),
                                        &no_physics, "disable-physics"));
