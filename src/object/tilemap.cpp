      ...
      add_path(false)
{
  ...
  if (width < 0 || height < 0) {
    ...
    height = 0;
    tiles.clear();
    resize(Sector::current()->get_width() / 32,
           Sector::current()->get_height() / 32);
    editor_active = false;
  }
  else {
...
void
TileMap::float_channel(float target, float& current, float remaining_time,
                       float elapsed_time)
{
...
TileMap::get_settings()
{
  ...
  ObjectOption draw_target_option(MN_STRINGSELECT, _("Draw target"),
                                  &draw_target);
  ...
  result.options.push_back(
      ObjectOption(MN_TOGGLE, _("Following path"), &add_path));
...
TileMap::update(float elapsed_time)
{
  ...
  if (current_tint.red != tint.red || current_tint.green != tint.green ||
    ...
    remaining_tint_fade_time =
        std::max(0.0f, remaining_tint_fade_time - elapsed_time);
    if (remaining_tint_fade_time == 0.0f) {
      current_tint = tint;
    }
    else {
      float_channel(tint.red, current_tint.red, remaining_tint_fade_time,
                    elapsed_time);
      float_channel(tint.green, current_tint.green, remaining_tint_fade_time,
                    elapsed_time);
      float_channel(tint.blue, current_tint.blue, remaining_tint_fade_time,
                    elapsed_time);
      float_channel(tint.alpha, current_tint.alpha, remaining_tint_fade_time,
                    elapsed_time);
    }
  }
...
TileMap::draw(DrawingContext& context)
{
  ...
  if (t_draw_rect.is_valid()) {
    ...
    for (pos.x = start.x, tx = t_draw_rect.left; tx < t_draw_rect.right;
         pos.x += 32, ++tx) {
      for (pos.y = ex_start.y, ty = ex_top; ty < t_draw_rect.top;
           pos.y += 32, ++ty) {
        int index = ty * width + tx;
        assert(index >= 0);
        assert(index < (width * height));
    ...
    for (pos.x = ex_start.x, tx = ex_left; tx < t_draw_rect.right;
         pos.x += 32, ++tx) {
      for (pos.y = ex_start.y, ty = ex_top; ty < t_draw_rect.bottom;
           pos.y += 32, ++ty) {
        int index = ty * width + tx;
        assert(index >= 0);
        assert(index < (width * height));
             ...
             int new_z_pos, bool newsolid)
{
  ...
  for (const auto& tile : tiles) tileset->get(tile);
}
