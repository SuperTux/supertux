...
Camera::update_scroll_normal(float dt_sec)
{
  ...
  Player& player = *Sector::get().get_players()[0];
  ...
  if (player.is_dying() || get_parent()->get_height() == 19*32) {
    ymode = 0;
  }
  if (ymode == 1) {
  ...
  if (ymode == 4) {
    ...
    if (player_pos.y > get_parent()->get_width() - upperend) {
      m_lookahead_pos.y = lowerend;
    }
  ...
  if (xmode == 2) {
    ...
    if (m_lookahead_mode == LookaheadMode::NONE) {
      ...
      if (player_pos.x < m_screen_size.width * 0.5f) {
        m_lookahead_mode = LookaheadMode::RIGHT;
      } else if (player_pos.x >= get_parent()->get_width() - m_screen_size.width * 0.5f) {
        m_lookahead_mode = LookaheadMode::LEFT;
      }
  ...
  if (xmode == 4) {
    ...
    if (player_pos.x > get_parent()->get_width() - LEFTEND) {
      m_lookahead_pos.x = RIGHTEND;
    }
