...
Explosion::explode()
{
  ...
  if (push) {
    ...
    auto near_objects = Sector::get().get_nearby_objects (center, 128.0 * 128.0);
    ...
    for (auto& obj: near_objects) {
	  ...
	  // If we somehow get a force of over 9000, keep it at 9000 because unexpected behaviour could result otherwise.
	  if (force > 9000.0f)
        force = 9000.0;
