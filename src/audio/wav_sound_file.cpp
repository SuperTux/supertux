...
WavSoundFile::WavSoundFile(PHYSFS_file* file_) : file(file_), datastart()
{
  ...
  do {
    if (PHYSFS_readBytes(file, chunkmagic, sizeof(chunkmagic)) <
        static_cast<std::make_signed<size_t>::type>(sizeof(chunkmagic)))
...
WavSoundFile::read(void* buffer, size_t buffer_size)
{
  ...
  if (PHYSFS_readBytes(file, buffer, readsize) !=
      static_cast<std::make_signed<size_t>::type>(readsize))
