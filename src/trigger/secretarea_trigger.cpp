...
SecretAreaTrigger::get_settings()
{
  ...
  result.options.push_back(ObjectOption(MN_TEXTFIELD, _("Fade-tilemap"),
                                        &fade_tilemap, "fade-tilemap",
                                        (OPTION_VISIBLE)));
  result.options.push_back(
      ObjectOption(MN_TEXTFIELD, _("Message"), &message, "message"));
...
SecretAreaTrigger::draw(DrawingContext& context)
{
  ...
  if (Editor::is_active()) {
    context.draw_filled_rect(bbox, Color(0.0f, 1.0f, 0.0f, 0.6f), 0.0f,
                             LAYER_OBJECTS);
  }
