...
class DrawingContext {
 public:
  ...
  void draw_line(const Vector& pos1, const Vector& pos2, const Color& color,
                 int layer);
  void draw_triangle(const Vector& pos1, const Vector& pos2, const Vector& pos3,
                     const Color& color, int layer);
  ...
  const Vector& get_translation() const { return transform.translation; }
 ...
 private:
  class Transform {
   public:
    ...
    Vector apply(const Vector& v) const { return v - translation; }
  };
 ...
 private:
  ...
  bool screenshot_requested; /**< true if a screenshot should be taken after the
