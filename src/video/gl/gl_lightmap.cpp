...
GLLightmap::get_light(const DrawingRequest& request) const
{
  ...
  float posX = request.pos.x * m_lightmap_width / viewport.get_screen_width() + rect.left;
  float posY = static_cast<float>((rect.get_height() * 1.0) + (rect.top * 1.0) - request.pos.y * m_lightmap_height / viewport.get_screen_height());
