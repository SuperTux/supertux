...
Canvas::clear()
{
  for (const auto& [_, requests] : m_requests)
  {
    for (const DrawingRequest* request : requests)
...
Canvas::render(Renderer& renderer, Filter filter)
{
  ...
  for (const auto& [layer, requests] : m_requests)
  {
    if (filter == BELOW_LIGHTMAP && layer >= LAYER_LIGHTMAP)
      break;
    if (filter == ABOVE_LIGHTMAP && layer <= LAYER_LIGHTMAP)
      continue;

    for (const DrawingRequest* req : requests)
    {
      const DrawingRequest& request = *req;
                     ...
                     int layer)
{
  ...
  m_requests[layer].push_back(request);
}
...
Canvas::get_pixel(const Vector& position, const std::shared_ptr<Color>& color_out)
{
  ...
  m_requests[LAYER_GETPIXEL].push_back(request);
}
