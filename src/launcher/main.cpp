...
public:
  ...
  ~ConfigSubsystem()
  {
    if (g_config)
    {
      try
      {
        // DO NOT SAVE: Some values are edited
        //g_config->save();
      }
...
void
LauncherMain::resave(const std::string& input_filename, const std::string& output_filename)
{
...
void
LauncherMain::launch_game(const CommandLineArguments& args)
{
  ...
  // OVERRIDES - That's why we must not save the config back
  g_config->window_size.width = 640;
  g_config->window_size.height = 400;
  ...
  bool launch_the_game_afterwards = false;
  screen_manager.push_screen(std::make_unique<LauncherMainScreen>(launch_the_game_afterwards));
  // launch_the_game_afterwards is unused
