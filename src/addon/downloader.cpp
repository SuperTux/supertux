...
class Transfer {
 ...
 public:
  ...
  TransferStatusPtr get_status() const { return m_status; }

  const char* get_error_buffer() const { return m_error_buffer.data(); }

  TransferId get_id() const { return m_id; }

  CURL* get_curl_handle() const { return m_handle; }
 ...
 private:
  static size_t on_data_wrap(char* ptr, size_t size, size_t nmemb,
                             void* userdata)
  {
    return static_cast<Transfer*>(userdata)->on_data(ptr, size, nmemb);
  }

  static int on_progress_wrap(void* userdata, double dltotal, double dlnow,
                              double ultotal, double ulnow)
  {
    return static_cast<Transfer*>(userdata)->on_progress(dltotal, dlnow,
                                                         ultotal, ulnow);
  }
    ...
    switch (msg->msg) {
      case CURLMSG_DONE: {
        ...
        if (resultfromcurl == CURLE_OK) {
          ...
          log_warning << "Error: " << curl_easy_strerror(resultfromcurl)
                      << std::endl;
          for (auto& callback : status->callbacks) {
            try {
              callback(false);
