...
class MD5 {
 public:
  ...
  void update(
      uint8_t* input,
      unsigned int
          input_length); /**< MD5 block update operation. Continues an MD5
 ...
 private:
  ...
  void transform(
      uint8_t* buffer); /**< MD5 basic transformation. Transforms state based on
                           block. Does the real update work.  Note that length
                           is implied to be 64. */
  ...
  static inline void FF(
      uint32_t& a, uint32_t b, uint32_t c, uint32_t d, uint32_t x, uint32_t s,
      uint32_t ac); /**< FF, GG, HH, and II transformations for rounds 1, 2, 3,
