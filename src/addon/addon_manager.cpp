...
static Addon&
get_addon(const AddonManager::AddonList& list, const AddonId& id,
          bool installed)
{
...
static std::vector<AddonId>
get_addons(const AddonManager::AddonList& list)
{
  std::vector<AddonId> results;
  results.reserve(list.size());
  std::transform(
      list.begin(), list.end(), std::back_inserter(results),
      [](const std::unique_ptr<Addon>& addon) { return addon->get_id(); });
  return results;
}
      ...
      m_transfer_status()
{
  if (!PHYSFS_mkdir(m_addon_directory.c_str())) {
    ...
    msg << "Couldn't create directory for addons '" << m_addon_directory
...
AddonManager::request_install_addon(const AddonId& addon_id)
{
  if (m_transfer_status) {
    ...
    m_transfer_status->then([this, install_filename, addon_id](bool success) {
      m_transfer_status = {};
      ...
      if (success) {
        ...
        if (repository_addon.get_md5() != md5.hex_digest()) {
          if (PHYSFS_delete(install_filename.c_str()) == 0) {
            log_warning << "PHYSFS_delete failed: " << PHYSFS_getLastErrorCode()
                        << std::endl;
          }
...
AddonManager::enable_addon(const AddonId& addon_id)
{
  ...
  if (addon.is_enabled()) {
    ...
    if (PHYSFS_mount(addon.get_install_filename().c_str(), mountpoint.c_str(),
                     0) == 0) {
      log_warning << "Could not add " << addon.get_install_filename()
                  << " to search path: " << PHYSFS_getLastErrorCode()
                  << std::endl;
    }
    else {
  ...
  if (!addon.is_enabled()) {
    ...
    if (PHYSFS_unmount(addon.get_install_filename().c_str()) == 0) {
      log_warning << "Could not remove " << addon.get_install_filename()
                  << " from search path: " << PHYSFS_getLastErrorCode()
                  << std::endl;
    }
    else {
      if (addon.get_type() == Addon::LANGUAGEPACK) {
        PHYSFS_enumerate(addon.get_id().c_str(), remove_from_dictionary_path,
                         NULL);
      }
...
AddonManager::is_old_enabled_addon(const std::unique_ptr<Addon>& addon) const
{
  return addon->get_format() == Addon::ORIGINAL &&
         addon->get_type() != Addon::LANGUAGEPACK && addon->is_enabled();
}
...
AddonManager::mount_old_addons()
{
  ...
  for (auto& addon : m_installed_addons) {
    if (is_old_enabled_addon(addon)) {
      if (PHYSFS_mount(addon->get_install_filename().c_str(),
                       mountpoint.c_str(), 0) == 0) {
        log_warning << "Could not add " << addon->get_install_filename()
                    << " to search path: " << PHYSFS_getLastErrorCode()
                    << std::endl;
      }
        ...
        if (addon_node.get_name() != "supertux-addoninfo") {
          ...
          try {
            std::unique_ptr<Addon> addon =
                Addon::parse(addon_node.get_mapping());
            m_addons.push_back(std::move(addon));
          }
          catch (const std::exception& e) {
            log_warning << "Problem when reading Add-on entry: " << e.what()
                        << std::endl;
          }
        }
      }
