...
StringArrayMenu::menu_action(MenuItem& item)
{
  ...
  if (id >= 0)
  {
    ...
    m_selected_item = id;
    get_item_by_id(-2).set_text(fmt::format(fmt::runtime(_("Selected item: %s")),
                                            (m_selected_item >= 0 ? m_array_items[m_selected_item] : _("None"))));
  }
...
StringArrayMenu::reload()
{
  ...
  add_entry(-2, fmt::format(fmt::runtime(_("Selected item: %s")),
                            (m_selected_item >= 0 ? m_array_items[m_selected_item] : _("None"))));
  add_entry(-3, _("Add"));
