...
MenuItem*
Menu::add_string_select(int id, const std::string& text, int* selected,
...
MenuItem*
Menu::add_file(const std::string& text, std::string* input,
...
{
  std::unique_ptr<ItemColorChannel> item(
      new ItemColorChannel(input, channel, id));
  return add_item(std::move(item));
}
...
MenuItem*
Menu::add_badguy_select(const std::string& text,
                        std::vector<std::string>* badguys, int id)
{
  std::unique_ptr<ItemBadguySelect> item(
      new ItemBadguySelect(text, badguys, id));
  return add_item(std::move(item));
}
...
Menu::process_input()
{
  ...
  if (menu_height > SCREEN_HEIGHT) {  // Scrolling
  ...
  if (controller->hold(Controller::REMOVE) && menu_repeat_time != 0 &&
      real_time > menu_repeat_time) {
    menuaction       = MENU_ACTION_REMOVE;
    menu_repeat_time = real_time + MENU_REPEAT_RATE;
  }
...
Menu::draw_item(DrawingContext& context, int index)
{
  ...
  if (active_item == index) {
    ...
    context.draw_filled_rect(
        Rectf(Vector(pos.x - menu_width_ / 2 + 10 - 2, y_pos - 12 - 2),
              Vector(pos.x + menu_width_ / 2 - 10 + 2, y_pos + 12 + 2)),
        Color(1.0f, 1.0f, 1.0f, blink), 14.0f, LAYER_GUI - 10);
    context.draw_filled_rect(
        Rectf(Vector(pos.x - menu_width_ / 2 + 10, y_pos - 12),
  ...
  switch (ev.type) {
    ...
    case SDL_TEXTINPUT:
      if (((ev.type == SDL_KEYDOWN && ev.key.keysym.sym == SDLK_BACKSPACE) ||
          ...
          items[active_item]->changes_width()) {
