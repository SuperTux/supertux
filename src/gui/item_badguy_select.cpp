...
ItemBadguySelect::ItemBadguySelect(const std::string& text_,
                                   std::vector<std::string>* badguys_, int _id)
    : MenuItem(text_, _id), badguys(badguys_)
{
}
...
ItemBadguySelect::process_action(const MenuAction& action)
{
  if (action == MENU_ACTION_HIT) {
    MenuManager::instance().push_menu(
        std::unique_ptr<Menu>(new BadguySelectMenu(badguys)));
  }
}
