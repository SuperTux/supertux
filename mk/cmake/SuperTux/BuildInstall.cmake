...
elseif(WIN32 AND CMAKE_HOST_WIN32)
  ...
  if(ANDROID)
    ...
    function(install_target target)
      get_target_property(_targetAlias ${target} ALIASED_TARGET)
      if(NOT _targetAlias)
        install(TARGETS ${target})
      else()
        install(TARGETS ${_targetAlias})
      endif()
    endfunction()

    install_target(SDL2)
    install_target(SDL2_image)
    install_target(Ogg)
    install_target(Vorbis)
    install_target(VorbisFile)
    install_target(libcurl)
    install_target(OpenAL)
    install_target(fmt)
    install_target(glm)
    install_target(PhysFS)
    install_target(PNG)
    install_target(ZLIB)
    install_target(Freetype)
    install_target(RAQM)
  endif()
endif()
